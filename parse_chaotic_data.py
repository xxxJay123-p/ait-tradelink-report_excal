...
def parse_html_data(html_data):
    ...
    for row in rows:
        ...
        if len(cells) == 6:
            ...
            # Check if the item description contains the "Form Type" information
            form_type = ""
            if "Form Type" in item_description:
                try:
                    form_type = item_description.split("<font")[1].split(">")[1].strip("</font>")
                    item_description = item_description.split("<div")[0].strip()
                except IndexError:
            ...
            # Extract the transaction ID from the item description
            transaction_id = item_description.split(' (')[1][:-1]

            # Determine the transaction type
            if "TDEC TRANSACTION" in item_description:
                transaction_type = "TDEC TRANSACTION"
                tdec_transaction = str(unit_price)
                import_export_charge = "0.0"
                tdec_clothing_levy = "0.0"
                tdec_penalty = "0.0"
                total = str(unit_price + float(net_amount_text))
            else:
...
# Call the parse_html_data function with the HTML data
html_data = """
...
structured_data = parse_html_data(html_data)
